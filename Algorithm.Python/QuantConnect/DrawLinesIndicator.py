...
class DrawLinesIndicator(object):
    ...
    def Update(self, bar):
        ...
        if self.Sar.CurrentColor == '红' and self.Sar.PreviousColor == "绿":
            point_status = "Peak"
            this_point = dict(x=self.Index, y=bar.Close)
        ...
        if point_status == "Peak" and len(self.Peaks) > 3 and len(self.Valleys) > 2 \
            and self.Peaks[-3]['y'] > self.Peaks[-2]['y'] > self.Peaks[-1]['y'] and self.Peaks[-2]['y'] < self.Peaks[-1]['y']:
    ...
    def CalPredicate(self, idx):
        if self.currentLineParams:
            return self.currentLineParams['k'] * idx + self.currentLineParams['b']
        return None
